package com.xiaozhu.common.refresh;
...
import com.xiaozhu.common.refresh.api.DefaultRefreshFooterCreator;
import com.xiaozhu.common.refresh.api.DefaultRefreshHeaderCreator;
...
import com.xiaozhu.common.refresh.constant.DelayedRunnable;
import com.xiaozhu.common.refresh.constant.DimensionStatus;
import com.xiaozhu.common.refresh.constant.RefreshState;
import com.xiaozhu.common.refresh.constant.SpinnerStyle;
import com.xiaozhu.common.refresh.constant.ViscousFluidInterpolator;
import com.xiaozhu.common.refresh.footer.BallPulseFooter;
import com.xiaozhu.common.refresh.header.BezierRadarHeader;
import com.xiaozhu.common.refresh.impl.RefreshContentWrapper;
import com.xiaozhu.common.refresh.impl.RefreshFooterWrapper;
import com.xiaozhu.common.refresh.impl.RefreshHeaderWrapper;
import com.xiaozhu.common.refresh.listener.OnLoadMoreListener;
import com.xiaozhu.common.refresh.listener.OnMultiPurposeListener;
...
import com.xiaozhu.common.refresh.listener.OnStateChangedListener;
...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent, NestedScrollingChild {
    ...
    public SmartRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        DisplayUtils density = new DisplayUtils();
    ...
    protected void onAttachedToWindow() {
        ...
        if (!thisView.isInEditMode()) {
            ...
            if (mRefreshContent == null) {
                final int padding = DisplayUtils.dip2px(20);
        ...
        public void run() {
            if (animationRunnable == this && !mState.isFinishing) {
                ...
                if (Math.abs(velocity) >= 1) {
                    ...
                    animationRunnable = null;
                    if (Math.abs(mSpinner) >= Math.abs(mSmoothDistance)) {
                        int duration = 10 * Math.min(Math.max((int) DisplayUtils.px2dip(Math.abs(mSpinner - mSmoothDistance)), 30), 100);
    ...
    public SmartRefreshLayout setFooterHeight(float heightDp) {
        if (mFooterHeightStatus.canReplaceWith(DimensionStatus.CodeExact)) {
            mFooterHeight = DisplayUtils.dip2px(heightDp);
    ...
    public SmartRefreshLayout setHeaderHeight(float heightDp) {
        if (mHeaderHeightStatus.canReplaceWith(DimensionStatus.CodeExact)) {
            mHeaderHeight = DisplayUtils.dip2px(heightDp);
    ...
    public SmartRefreshLayout setHeaderInsetStart(float insetDp) {
        mHeaderInsetStart = DisplayUtils.dip2px(insetDp);
        return this;
    }
    ...
    public SmartRefreshLayout setFooterInsetStart(float insetDp) {
        mFooterInsetStart = DisplayUtils.dip2px(insetDp);
        return this;
    }
