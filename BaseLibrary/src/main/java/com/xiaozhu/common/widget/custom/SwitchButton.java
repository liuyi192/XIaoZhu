...
public class SwitchButton extends View implements ValueAnimator.AnimatorUpdateListener {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (deltaX == 0 || deltaY == 0 || deltaX > mTouchSlop) {
                    ...
                    SpringAnim(ss.getX(), toggleOn ? spotMaxX : spotMinX);
                    if (listener != null) {
    ...
    private void ChangeCBAnim() {
        if (!toggleOn) {
            PropertyValuesHolder pvhY = PropertyValuesHolder.ofFloat("scale", 1f, 0f);
            ObjectAnimator alpAnim = ObjectAnimator.ofPropertyValuesHolder(bb, pvhY);
    ...
    private void SpringAnim(float startX, float endX) {
        ...
        if (!toggleOn) {
            PropertyValuesHolder pvhY = PropertyValuesHolder.ofFloat("scale", 0f, 1f);
        ...
        if (startX != endX) {
             ...
             * 小球滑到目标位置的动画
             */
            ObjectAnimator sAnim = ObjectAnimator.ofFloat(ss, "x", startX, endX - back);
            // 设置sAnim动画的插值方式：加速插值
             ...
             * 小球弹到最终位置
             */
            ObjectAnimator springAnim1 = ObjectAnimator.ofFloat(ss, "x", endX - back, endX);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.UNSPECIFIED || widthMode == MeasureSpec.AT_MOST) {
            widthSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 60, r.getDisplayMetrics());
